...
const tweaks = TweaksManager.getSiteTweaks(getCurrentUrl());
...
function initLTAssistant() {
  if (currentDocumentElement = document.documentElement, !currentDocumentElement) return;
  ...
  window.ltAssistant = new LTAssistant(e, tweaks), window.clearInterval(interval), interval = window.setInterval((() => {
...
function onGetSelectedTextMessage(e, t) {
  ...
  const n = tweaks.getSelectedText(), s = tweaks.getSelectedHTML();
  if (n) {
    const e = {initialCommand: "GET_SELECTED_TEXT", isSuccessful: !0, selectedText: n, selectedHTML: s};
